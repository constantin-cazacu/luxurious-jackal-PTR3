defmodule RetweetAnalysis.LoadBalancer do
  use GenServer
  require Logger
...
#      IO.inspect("[Retweet Load Balancer]> The process is not alive")
      ...
      if list_length > 0 do
        ...
        DynamicSupervisor.terminate_child(RetweetAnalysis.PoolSupervisor, pid_to_kill)
      end
    end
  end
  ...
  def handle_cast({:receive_tweet, {id, tweet}}, state) do
    ...
    if length(worker_list) > 0 do
      ...
      RetweetAnalysis.Worker.receive_tweet(worker_pid, tweet)
    end
...
#    IO.inspect("Retweet Load Balancer: Workers to kill #{inspect(list_of_workers_to_kill)}")
